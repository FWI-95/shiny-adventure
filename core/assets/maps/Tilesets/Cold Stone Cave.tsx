<?xml version="1.0" encoding="UTF-8"?>
<tileset name="Cold Stone Cave" tilewidth="48" tileheight="48" tilecount="256" columns="16">
 <image source="../Images/Cold Stone Cave.png" trans="ff01fe" width="768" height="768"/>
 <terraintypes>
  <terrain name="Base Cliff" tile="33"/>
 </terraintypes>
 <tile id="0" terrain=",,,0"/>
 <tile id="1" terrain=",,0,0"/>
 <tile id="2" terrain=",,0,"/>
 <tile id="3" terrain=",,,0"/>
 <tile id="4" terrain=",,0,0"/>
 <tile id="5" terrain=",,0,"/>
 <tile id="16" terrain=",0,,0"/>
 <tile id="17" terrain="0,0,0,0"/>
 <tile id="18" terrain="0,,0,"/>
 <tile id="19" terrain=",0,,0"/>
 <tile id="20" terrain="0,0,0,0"/>
 <tile id="21" terrain="0,,0,"/>
 <tile id="32" terrain=",0,,"/>
 <tile id="33" terrain="0,0,,"/>
 <tile id="34" terrain="0,,,"/>
 <tile id="35" terrain=",0,,"/>
 <tile id="36" terrain="0,0,,"/>
 <tile id="37" terrain="0,,,"/>
 <tile id="132" terrain=",0,0,0"/>
 <tile id="133" terrain="0,,0,0"/>
 <tile id="148" terrain="0,0,,0"/>
 <tile id="149" terrain="0,0,0,"/>
 <tile id="214" terrain="0,,,0"/>
 <tile id="230" terrain=",0,0,"/>
</tileset>
